...
export default function BridgePage() {
  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--ab-navy)" }}>
      ...
      {/* Hero */}
      <div style={{
        background: "linear-gradient(135deg, var(--ab-navy) 0%, var(--ab-royal) 50%, var(--ab-sky) 100%)",
        padding: "3rem 2rem", textAlign: "center",
      }}>
        <p style={{ fontFamily: "var(--font-display)", fontWeight: 800, fontSize: "clamp(24px, 4vw, 40px)", color: "var(--ab-text-primary)", margin: "0 0 8px" }}>
          🌉 ArtemisBank
        </p>
        <p style={{ fontSize: "16px", color: "var(--ab-text-secondary)", margin: "0 0 4px" }}>
          Bridge USDC to Arc Testnet in seconds
        </p>
        <p style={{ fontSize: "13px", color: "var(--ab-text-secondary)", margin: 0 }}>
      ...
      <div style={{ maxWidth: "520px", margin: "0 auto", padding: "2rem" }}>
        ...
        {!isConnected ? (
          <div style={{
            ...
            <p style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "18px", color: "var(--ab-text-primary)", margin: "0 0 8px" }}>
              Connect Your Wallet
            </p>
            <p style={{ color: "var(--ab-text-secondary)", fontSize: "14px", margin: "0 0 24px" }}>
          ...
          <div style={{
            background: "var(--ab-royal)", border: "0.5px solid rgba(34,197,94,0.3)",
            ...
            <p style={{ color: "var(--ab-text-primary)", fontSize: "14px", margin: "0 0 24px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <a href="/wallet" style={{
                ...
                background: "var(--ab-sky)", color: "var(--ab-navy)",
              ...
              <button
                ...
                style={{
                  ...
                  border: "1px solid var(--ab-border)", background: "var(--ab-navy)",
                  color: "var(--ab-text-primary)", fontFamily: "var(--font-display)",
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
            ...
            {/* Bridge Form */}
            <div style={{ background: "var(--ab-royal)", border: "0.5px solid var(--ab-border)", borderRadius: "20px", padding: "1.5rem" }}>
              ...
              <div style={{ marginBottom: "1rem" }}>
                <label style={{ fontSize: "12px", color: "var(--ab-text-secondary)", fontWeight: 500, display: "block", marginBottom: "6px", textTransform: "uppercase", letterSpacing: "0.05em" }}>
                  From
                </label>
                <div style={{ display: "flex", gap: "8px" }}>
                  {SUPPORTED_SOURCES.map(chain => (
                    <button key={chain.id} onClick={() => setSourceChain(chain)} style={{
                      ...
                      border: sourceChain.id === chain.id ? "2px solid var(--ab-sky)" : "1px solid var(--ab-border)",
                      background: sourceChain.id === chain.id ? "var(--ab-navy)" : "var(--ab-ice)",
                      cursor: "pointer", textAlign: "center", transition: "all 0.15s",
                    }}>
                      <p style={{ fontSize: "18px", margin: "0 0 2px" }}>{chain.emoji}</p>
                      <p style={{ fontSize: "10px", fontWeight: 600, color: sourceChain.id === chain.id ? "var(--ab-sky)" : "var(--ab-text-secondary)", margin: 0 }}>
                        {chain.label.split(" ")[0]}
                      </p>
                    </button>
                  ))}
              ...
              <div style={{ marginBottom: "1rem" }}>
                ...
                <div style={{
                  ...
                  <div>
                    <p style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "14px", color: "var(--ab-text-primary)", margin: 0 }}>Arc Testnet</p>
                    <p style={{ fontSize: "11px", color: "var(--ab-text-secondary)", margin: 0 }}>Chain ID: 5042002 · Native USDC</p>
                  </div>
                  <span style={{ marginLeft: "auto", fontSize: "11px", color: "var(--ab-win)", fontWeight: 600, background: "rgba(34,197,94,0.1)", padding: "2px 8px", borderRadius: "20px" }}>
                    ✓ Destination
                  </span>
                </div>
              </div>
              ...
              <div style={{ marginBottom: "1rem" }}>
                ...
                <div style={{ position: "relative", marginBottom: "8px" }}>
                  ...
                  />
                  <span style={{ position: "absolute", right: "14px", top: "50%", transform: "translateY(-50%)", fontSize: "13px", fontWeight: 600, color: "var(--ab-sky)" }}>
                    USDC
                  </span>
                </div>
                <div style={{ display: "flex", gap: "6px" }}>
                  {quickAmounts.map(v => (
                    <button key={v} onClick={() => setAmount(v)} disabled={bridging} style={{
                      ...
                      background: amount === v ? "rgba(6,182,212,0.08)" : "var(--ab-navy)",
              ...
              {/* Info box */}
              <div style={{ background: "var(--ab-ice)", borderRadius: "10px", padding: "10px 14px", marginBottom: "1rem", fontSize: "12px", color: "var(--ab-muted)", lineHeight: "1.6" }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: bridging ? "rgba(6,182,212,0.4)" : "var(--ab-sky)",
                  color: "var(--ab-navy)", fontFamily: "var(--font-display)", fontWeight: 700,
                  ...
                  {STEPS.map((step, i) => {
                    ...
                    return (
                      <div key={step.key} style={{
                        ...
                        background: status === "done" ? "rgba(34,197,94,0.06)" : status === "loading" ? "rgba(6,182,212,0.06)" : status === "error" ? "rgba(239,68,68,0.06)" : "var(--ab-ice)",
                        border: `0.5px solid ${status === "done" ? "rgba(34,197,94,0.2)" : status === "loading" ? "rgba(6,182,212,0.2)" : status === "error" ? "rgba(239,68,68,0.2)" : "transparent"}`,
                        transition: "all 0.3s",
                      }}>
                        <div style={{
                          width: "28px", height: "28px", borderRadius: "50%", flexShrink: 0,
                          display: "flex", alignItems: "center", justifyContent: "center",
                          background: status === "done" ? "var(--ab-win)" : status === "loading" ? "var(--ab-sky)" : status === "error" ? "var(--ab-loss)" : "rgba(6,182,212,0.15)",
                          fontSize: "12px", fontWeight: 700, color: status === "idle" ? "var(--ab-text-secondary)" : "var(--ab-navy)",
                        }}>
                          {status === "done" ? "✓" : status === "loading" ? "…" : status === "error" ? "✕" : i + 1}
                        </div>
                        <div style={{ flex: 1 }}>
                          <p style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: "13px", color: "var(--ab-text-primary)", margin: 0 }}>{step.label}</p>
                          <p style={{ fontSize: "11px", color: "var(--ab-text-secondary)", margin: 0 }}>{step.desc}</p>
                        </div>
                        {txLinks[step.key] && (
                          <a href={txLinks[step.key]} target="_blank" rel="noopener noreferrer" style={{ fontSize: "11px", color: "var(--ab-sky)", fontWeight: 600 }}>
                            View →
                          </a>
                        )}
                      </div>
            ...
            {/* What's next */}
            <div style={{ background: "var(--ab-royal)", border: "0.5px solid var(--ab-border)", borderRadius: "16px", padding: "1.25rem" }}>
              <p style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "14px", color: "var(--ab-text-primary)", margin: "0 0 10px" }}>
                After bridging:
              </p>
                ...
                {[
                  ...
                  <div key={item.step} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                    <div style={{
                      ...
                      fontSize: "11px", fontWeight: 700, color: "var(--ab-sky)",
                    }}>
                      {item.step}
                    </div>
                    <p style={{ fontSize: "13px", color: "var(--ab-text-primary)", margin: 0 }}>{item.text}</p>
                  </div>
                ))}
              </div>
            </div>
